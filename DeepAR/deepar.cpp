...
#define BOUND 0 // 0 - Adapart, 1 - Huber-Law, 2 - Huber's integer bound
#define TASSA 0 // Ensure the matrix has total support (0/1) 
#define DOUBLY_STOCHASTIC 0 // Make the matrix nearly doubly stochastic (0/1)
...
long double huber_precomputed[MAX_N + 1];
...
long double huber_h(long double r) {
	...
	if (BOUND == 1) {
		...
		return huber_precomputed[(int)(r + EPSILON)] / EULER;
	}
}
...
void precompute(int n) {
	gen.seed(time(0));
	srand(time(0));
	huber_precomputed[0] = 0;
	huber_precomputed[1] = EULER;
	for (int i = 0; i <= n; i++) {
		gamma_precomputed[i] = exp(row_bound(i));
		if (i >= 2) {
			huber_precomputed[i] = huber_precomputed[i - 1] + 1 + 0.5 / huber_precomputed[i - 1] + 0.6 / huber_precomputed[i - 1] / huber_precomputed[i - 1];
		}
	}
...
long double hybrid_bound(Matrix<T> matrix, int J) {
	if (J <= 0) return BOUND ? huber_law_bound(matrix) : minc_bregman(matrix);
	int n = matrix.size();
	return dp[n - 1][(1<<J) - 1];
}
...
int main() {
	...
	if (DOUBLY_STOCHASTIC) {
		if (!TASSA) {
			cout<<"DOUBLY_STOCHASTIC cannot be 1 if TASSA is 0"<<endl;
			return 0;
		}
		{
	...
	while (S != K) {
		S += rejection_sample(matrix, J);
		R += distribution(generator);
		if (time_since(start_time) > time_limit) {
			cout<<"Execution timed out"<<endl;
			return 0;
		}
	}
	...
	cout<<(coef + ub + log(p))<<" "<<preprocessing_time<<" "<<sampling_time<<" "<<time_since(start_time)<<endl;
}